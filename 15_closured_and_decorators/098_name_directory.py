...
Each person has a first name, last name, age and sex. Print their names in a specific format sorted by their age
...
def person_lister(f):
    def inner(people):
        return (f(p) for p in sorted(people, key=operator.itemgetter(2)))
    return inner
